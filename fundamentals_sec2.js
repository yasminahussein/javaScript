...
*/
//anther way to write the needed function 
/*
...
function calcTip(bills){
    for(let i =0 ; i< bills.length ; i++){
        if (bills[i] >=50 && bills[i] <=300){
            tips[i]=.15;
            totals[i]= bills[i]+(bills[i]*tips[i]);
            console.log(`the total value of bill(${bills[i]})=${totals[i]}`);
            ...
            console.log(`bill must be more than 50 your current bill is (${bills[i]})`);
            tips[i]=0;
            totals[i]= bills[i];
        }
    }
...
totals2=calcTip(bills);
 console.log("total values =" +totals2);
 */
 ...
 calcTip = function (bill) {
  ...
  function calcAverage2(arr){
      ...
      totalAverage= totalAverage/arr.length ;
      console.log(totalAverage);
      return totalAverage;
  }
      let result = calcAverage2(bills);
  console.log(result);